...
class DaySix
  def initialize(input_data)
    @test_input = split_lines(input_data)
  end

  def split_lines(input_data)
    split_lines = input_data.gsub(/ /, '').split(/\n{2,}/)
  end

  def solve_part_one
    count = 0
    @test_input.map { |x| x.gsub(/\n/, '') }.each do |group|
      count += group.split('').uniq.length
    end
    count
  end
  ...
  def solve_part_two
    count = 0
    @test_input.map { |x| x.split(/\n/) }.each do |group|
      group_size = group.length
      ('a'..'z').to_a.each do |letter|
        letter_count = 0
        group.each do |member|
          letter_count += 1 if member.include?(letter)
        end
        count += 1 if letter_count == group_size
      end
    end
    count
  end
